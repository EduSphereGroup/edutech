            ...
            <div className="flex items-center space-x-2 sm:space-x-4 lg:space-x-6">
              ...
              >
                <div className="flex items-center space-x-1 bg-gradient-to-r from-yellow-50 to-orange-50 px-2 sm:px-3 py-1.5 rounded-lg border border-yellow-200">
                  ...
                  <span className="text-xs sm:text-sm font-bold text-gray-700">{stats?.xp}</span>
                  <span className="text-xs text-gray-500 hidden sm:inline">XP</span>
                </div>
                {getXPToNextLevel(stats?.xp ?? 0) > 0 && (
                  <div className="text-xs text-gray-500 mt-0.5 flex items-center">
                    <Zap className="h-2.5 w-2.5 sm:h-3 sm:w-3 mr-0.5" />
                    <span className="hidden sm:inline">{getXPToNextLevel(stats?.xp ?? 0)} para nível {stats?.level ?? 1 + 1}</span>
                    <span className="sm:hidden">+{getXPToNextLevel(stats?.xp ?? 0)}</span>
                  </div>
                )}
              </motion.div>
              ...
              <motion.div
                ...
                className={`px-2 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium border ${getLevelColor(stats?.level ?? 0)}`}
              >
                <div className="flex items-center space-x-1 sm:space-x-2">
                  <Trophy className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                  <span className="hidden sm:inline">Nível {stats?.level ?? 0}</span>
                  <span className="sm:hidden">{stats?.level ?? 0}</span>
                </div>
              </motion.div>
